	...
	<div class="productbg">
		...
		<%
			int totalPrice=0;
				...
				allshoppinglistcombination=shoppingListCombinationDB.getShoppingListCombinationByMemberAccount(orderID);
				...
				allshoppinglistcustomerlize=shoppingListCustomerlizeDB.getShoppingListCustomerlizeByMemberAccount(orderID);				
		%>
		...
		<div class="container productcontent">
			...
			<table class="table table-hover cart_table">
				...
				<tbody>
					...
					<tr>
						<td><a
							href="productDetail.jsp?ID=<%=shoppingListBean.getProductID()%>">
								<img src="<%=shoppingListBean.getImage()%>" class="cart_product"
								alt=""> <%=shoppingListBean.getProductName()%>
						</a></td>

						<td><%=shoppingListBean.getQuantity()%></td>
						<td>$<%=shoppingListBean.getUnitPrice()%></td>
						<%
							totalPrice+=shoppingListBean.getQuantity()*shoppingListBean.getUnitPrice();
						%>
			...
			<!--客製化組裝區 -->
<%
				for(ShoppingListCustomerlizeBean shoppingListCustomerlizeBean:allshoppinglistcustomerlize){
			%>
			<table class="table table-hover cart_table">
				<thead>
					<tr>
				...
				<tbody>
					<tr>
						<td><a
							href="customerlizeDetail.jsp?ID=<%=shoppingListCustomerlizeBean.getCusID()%>">
								<img src="img/cusimg.jpg" class="cart_product"
								alt=""> 
						</a></td>
						<td><%=shoppingListCustomerlizeBean.getQuantity()%></td>
						<td>$<%=shoppingListCustomerlizeBean.getUnitPrice()%></td>
						<%
							totalPrice+=shoppingListCustomerlizeBean.getQuantity()*shoppingListCustomerlizeBean.getUnitPrice();
						%>
			...
			<!--優惠組合區 -->
			<%
				for(ShoppingListCombinationBean shoppingListcombinationBean:allshoppinglistcombination){
			%>
			<table class="table table-hover cart_table">
				<thead>
					<tr>
				...
				<tbody>
					<tr>
						<td><a
							href="combinationDetail.jsp?ID=<%=shoppingListcombinationBean.getCombinationID()%>">
								<img src="<%=shoppingListcombinationBean.getComImage()%>"
								class="cart_combination" alt=""> <%=shoppingListcombinationBean.getCombinationName()%>
						</a></td>
						<td><%=shoppingListcombinationBean.getQuantity()%></td>
						<td>$<%=shoppingListcombinationBean.getUnitPrice()%></td>
						<%
							totalPrice+=shoppingListcombinationBean.getQuantity()*shoppingListcombinationBean.getUnitPrice();
						%>
			...
			<div class="totalprice_div">
				<p>
					總金額: <span>$<%=totalPrice%></span> <input type="hidden" value=""
						name="totalPrice">
				</p>
			</div>
			<input class="btn btn-primary btn_cart" type="button" value="返回"
